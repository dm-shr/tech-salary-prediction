...
def log_seed_metrics_and_plot(results, mlflow, logger):
    ...
    r2_train_ci = np.array([mean_confidence_interval(r2_train_values[:, i]) 
                           for i in range(r2_train_values.shape[1])])
    r2_test_ci = np.array([mean_confidence_interval(r2_test_values[:, i]) 
                          for i in range(r2_test_values.shape[1])])
    mae_test_ci = np.array([mean_confidence_interval(mae_test_values[:, i]) 
                           for i in range(mae_test_values.shape[1])])
    rmse_test_ci = np.array([mean_confidence_interval(rmse_test_values[:, i]) 
                            for i in range(rmse_test_values.shape[1])])
    ...
    plt.fill_between(range(len(r2_train_mean[1:])), 
                    r2_train_ci[1:, 1], r2_train_ci[1:, 2], alpha=0.3)
    plt.plot(r2_test_mean[1:], label='test')
    plt.fill_between(range(len(r2_test_mean[1:])), 
                    r2_test_ci[1:, 1], r2_test_ci[1:, 2], alpha=0.3)
    ...
    logger.info(f'R2: mean = {r2_test_mean[best_epoch]:.4f}, ' 
               f'95% CI = [{r2_test_ci[best_epoch, 1]:.4f}, {r2_test_ci[best_epoch, 2]:.4f}]')
    logger.info(f'MAE: mean = {mae_test_mean[best_epoch]:.4f}, '
               f'95% CI = [{mae_test_ci[best_epoch, 1]:.4f}, {mae_test_ci[best_epoch, 2]:.4f}]')
    logger.info(f'RMSE: mean = {rmse_test_mean[best_epoch]:.4f}, '
